...
/** Drop the per-account keys the cross-account endpoint does not accept. */
function globalQuery(query: MessageQuery): GlobalMessageQuery {
  const { folder_id: _folder, thread_id: _thread, ...rest } = query;
  return rest;
}
...
export function useFolders(accountId: string | null): FoldersState {
  ...
  useEffect(() => {
    // "All accounts" has no folder list — folders belong to one mailbox.
    if (accountId === null || accountId === ALL_ACCOUNTS) {
      setFolders([]);
      setLoading(false);
      return;
    }
    ...
    api
      ...
      .catch((err: unknown) => {
        if (!cancelled)
          setError(
            err instanceof ApiError ? err.message : "Failed to load folders",
          );
      })
      .finally(() => {
        if (!cancelled) setLoading(false);
...
export function useMessages(
  ...
  useEffect(() => {
    ...
    const pending =
      accountId === ALL_ACCOUNTS
        ? api.searchMessages({ ...globalQuery(query), limit: 50 })
        : api.listMessages(accountId, {
            ...query,
            limit: 50,
            cursor: undefined,
          });
  ...
  const loadMore = useCallback(() => {
    ...
    const pending =
      accountId === ALL_ACCOUNTS
        ? api.searchMessages({ ...globalQuery(query), limit: 50, cursor })
        : api.listMessages(accountId, { ...query, limit: 50, cursor });
    pending
      .then((page) => {
...
export function useMessageDetail(messageId: string | null): MessageDetailState {
  ...
  const fetchOnce = useCallback(async (id: string) => {
    const [msg, body] = await Promise.all([
      api.getMessage(id),
      api.getBody(id),
    ]);
    setMessage(msg);

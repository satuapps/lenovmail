      ...
      <div className="border-b border-ink-600 p-2">
        ...
        {activeAccount !== null && (
          <div className="mt-2 flex items-center justify-between text-xs text-fg-muted">
            ...
            <span className={accountStatusChipClass(activeAccount.status)}>
              {ACCOUNT_STATUS_LABEL[activeAccount.status] ??
        ...
        {allAccounts && (
          <div className="mt-2 text-xs text-fg-muted">
            Searching {accounts.length} account
            {accounts.length === 1 ? "" : "s"}.
          </div>
        )}
      </div>
      <nav className="min-h-0 flex-1 overflow-y-auto p-2">
        ...
        {allAccounts ? (
          <p className="px-2 py-2 text-xs text-fg-muted">
            Folders are per-account — pick one account to browse folders.
          </p>
        ) : (
          <>
            <button
            ...
            {foldersLoading && (
              <p className="px-2 py-2 text-xs text-fg-dim">Loading folders…</p>
            )}
            {!foldersLoading &&
              folders.length === 0 &&
